...
export async function loadPdfData(
  ...
  const [
    ...
    admin.from("document_items").select("*").eq("document_id", documentId)
      .order("position"),
    admin
      .from("clients")
    ...
    admin.from("bank_accounts").select("*").eq("active", true).order(
      "display_order",
    ),
  ]);
  ...
  const selectedClient: RecordValue = Object.keys(clientSnapshot).length
    ? clientSnapshot
    : {
  ...
  const selectedSeller: RecordValue = Object.keys(sellerSnapshot).length
    ? sellerSnapshot
    : {
  ...
  const selectedBanks = Array.isArray(snapshotBanks)
    ? snapshotBanks
    : (banks ?? []);
  return {
    type: document.type,
    ...
    subtotalUsd: document.subtotal_usd == null
      ? null
