...
export type PdfItem = {
  ...
  subtotalDocument: string | null;
  taxDocument: string | null;
  totalDocument: string | null;
...
export type PdfData = {
  ...
  applyIgv: boolean;
  currency: 'USD' | 'PEN';
  exchangeRatePenPerUsd: string | null;
...
const money = (value: string | null | undefined, currency: 'USD' | 'PEN') =>
  value ? `${currency === 'PEN' ? 'S/' : 'USD'} ${Number(value).toFixed(2)}` : '—';
...
export async function createFravePdf(data: PdfData, draft = false): Promise<Uint8Array> {
  ...
  let y = 660;
  const { width } = page.getSize();
  const hasTotals = data.totalDocument !== null;
  ...
  if (data.currency === 'PEN' && data.exchangeRatePenPerUsd) {
    page.drawText(
      `Tasa usada: S/ ${Number(data.exchangeRatePenPerUsd).toFixed(4)} por USD${
        data.exchangeRateObservedAt ? ` · ${data.exchangeRateObservedAt}` : ''
      }`,
      { x: 320, y: y - 13, size: 7.1, font: regular, color: muted },
    );
    if (data.exchangeRateSource) {
      page.drawText(data.exchangeRateSource, {
        x: 320,
        y: y - 24,
        size: 6.5,
        font: regular,
  ...
  const proposalColumns = [
    ...
    { label: currencyLabel, x: 488, width: 70 },
  ];
  const confirmationColumns = [
    ...
    { label: totalLabel, x: 511, width: 55 },
  ];
  ...
  for (const item of data.items.filter((row) => row.sku || row.denomination)) {
    ...
    if (!hasTotals) {
      drawTableValue(
        page,
        bold,
        money(item.unitPriceDocument, data.currency).replace(
          data.currency === 'PEN' ? 'S/ ' : 'USD ',
          '',
        ),
        columns[3].x,
        baseY,
      ...
      drawTableValue(
        page,
        regular,
        money(item.subtotalDocument, data.currency).replace(
          data.currency === 'PEN' ? 'S/ ' : 'USD ',
          '',
        ),
        columns[5].x,
      ...
      drawTableValue(
        page,
        regular,
        money(item.taxDocument, data.currency).replace(
          data.currency === 'PEN' ? 'S/ ' : 'USD ',
          '',
        ),
        columns[6].x,
      ...
      drawTableValue(
        page,
        bold,
        money(item.totalDocument, data.currency).replace(
          data.currency === 'PEN' ? 'S/ ' : 'USD ',
          '',
        ),
        columns[7].x,
  ...
  if (hasTotals) {
    ...
    const totalRows: Array<[string, string | null]> = [
      ['Subtotal', data.subtotalDocument],
      ['TOTAL', data.totalDocument],
    ];
    if (hasIgv) totalRows.splice(1, 0, ['IGV', data.taxDocument]);
    totalRows.forEach(([label, amount], index) => {
      ...
      page.drawText(money(amount, data.currency), {
        x: 490,
        y: y - index * 17,
        size: isTotal ? 9 : 8,
        font: bold,
  ...
  if (data.type === 'proforma' && data.banks.length) {
    ...
    y -= 14;
    for (const bank of data.banks.filter((bank) => textOf(bank.currency) === data.currency)) {
      if (y < 65) break;
      page.drawText(
        `${textOf(bank.bank_name)} · ${textOf(bank.currency)} · ${textOf(
          bank.account_type,
        )} · ${textOf(bank.account_number)}${bank.cci ? ` · CCI ${textOf(bank.cci)}` : ''}`,
