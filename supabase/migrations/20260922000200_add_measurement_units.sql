-- Products can now be counted and priced in their own unit. The legacy column
-- names remain to preserve the existing inventory and document calculations.
create table public.measurement_units (
  id uuid primary key default gen_random_uuid(),
  name text not null,
  symbol text not null,
  normalized_name text generated always as (lower(trim(name))) stored,
  normalized_symbol text generated always as (lower(trim(symbol))) stored,
  active boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint measurement_units_name_not_blank check (nullif(trim(name), '') is not null),
  constraint measurement_units_symbol_not_blank check (nullif(trim(symbol), '') is not null),
  constraint measurement_units_symbol_length check (char_length(trim(symbol)) <= 8),
  unique (normalized_name),
  unique (normalized_symbol)
);

insert into public.measurement_units (name, symbol)
values ('Kilogramo', 'kg');

alter table public.products
  add column measurement_unit_id uuid references public.measurement_units(id);

update public.products
set measurement_unit_id = (select id from public.measurement_units where normalized_symbol = 'kg');

alter table public.products
  alter column measurement_unit_id set not null;

-- This default also protects imports or future integration code that does not
-- explicitly select a unit yet.
create or replace function public.assign_product_measurement_unit()
returns trigger
language plpgsql
security definer
...
as $$
begin
  if new.measurement_unit_id is null then
    select id into new.measurement_unit_id
    from public.measurement_units
    where normalized_symbol = 'kg' and active;
  end if;

  if not exists (
    select 1 from public.measurement_units
    where id = new.measurement_unit_id and active
  ) then
    raise exception 'La unidad de medida seleccionada no está disponible';
  end if;

  if tg_op = 'UPDATE' and new.measurement_unit_id is distinct from old.measurement_unit_id then
    if old.stock_kg <> 0 then
      raise exception 'No puedes cambiar la unidad de un producto con stock. Ajusta primero el stock a cero.';
    end if;
    if exists (select 1 from public.document_items where product_id = old.id) then
      raise exception 'No puedes cambiar la unidad de un producto que ya figura en documentos.';
    end if;
  end if;
...
create trigger products_assign_measurement_unit
before insert or update of measurement_unit_id on public.products
for each row execute function public.assign_product_measurement_unit();

create or replace function public.protect_used_measurement_unit_symbol()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if new.symbol is distinct from old.symbol
    and exists (select 1 from public.products where measurement_unit_id = old.id) then
    raise exception 'No puedes cambiar el símbolo de una unidad que ya está asignada a productos';
  end if;
  return new;
end;
$$;

create trigger measurement_units_protect_symbol
before update of symbol on public.measurement_units
for each row execute function public.protect_used_measurement_unit_symbol();
...
alter table public.document_items
  add column unit_snapshot text not null default 'kg'
    check (nullif(trim(unit_snapshot), '') is not null);
...
begin
  ...
  if parent_status = 'draft' then
    if new.source_quote_item_id is not null then
      ...
      new.unit_snapshot = source_item.unit_snapshot;
      new.unit_price_usd = source_item.unit_price_usd;
      new.unit_price_document = source_item.unit_price_document;
      return new;
    end if;

    select p.sku, p.name, p.unit_price_usd, c.name as category, unit.symbol as unit_symbol
      into product_row
      from public.products p
      join public.product_categories c on c.id = p.category_id
      join public.measurement_units unit on unit.id = p.measurement_unit_id
      where p.id = new.product_id and p.active and unit.active;
    ...
    new.unit_snapshot = product_row.unit_symbol;
...
begin
  ...
  if doc.type = 'proforma' then
    ...
    for stock_row in select product_id, sum(quantity_kg)::numeric(12,3) as required_kg from public.document_items where document_id = doc.id group by product_id order by product_id loop
      ...
      select symbol into product_unit_symbol from public.measurement_units where id = product_row.measurement_unit_id;
      if product_row.stock_kg < stock_row.required_kg then
        raise exception 'Stock insuficiente para %: disponible % %, requerido % %',
          product_row.sku, product_row.stock_kg, product_unit_symbol, stock_row.required_kg, product_unit_symbol;
      end if;
...
create policy measurement_units_read_active on public.measurement_units
